...
function createDialog() {
    ...
    customInputRadioButton.value = true;
    ...
    fullCSVButton.helpTip = "Export all lines from the CSV file. Disables any other forms of  export.";
    fullCSVButton.text = "Full CSV File";
    fullCSVButton.value = true;
    ...
    singleLineRadioButton.helpTip = "Export a single line from the CSV file. Disables any other forms of  export.";
    singleLineRadioButton.text = "Single Line";

    var singleLineText = singleLineGroup.add('edittext {justify: "center", properties: {name: "singleLineText"}}' as 'edittext');
    ...
    customRangeRadioButton.helpTip = "Export a range of lines from the CSV file. Disables any other forms of  export.";
    ...
    var startRangeText = rangeTextGroup.add('edittext {justify: "center", properties: {name: "startRangeText"}}' as 'edittext');
    startRangeText.helpTip = "Starting CSV line to export. Inclusive.";
    startRangeText.text = "Start";
    startRangeText.preferredSize.width = 55;
    startRangeText.alignment = ["left", "fill"];

    var statictext1 = rangeTextGroup.add("statictext", undefined, undefined, { name: "statictext1" });
    statictext1.text = "-";
    statictext1.alignment = ["left", "center"];

    var endRangeText = rangeTextGroup.add('edittext {justify: "center", properties: {name: "endRangeText"}}' as 'edittext');
    endRangeText.helpTip = "Ending CSV line to export. Inclusive.";
    endRangeText.text = "End";
    endRangeText.preferredSize.width = 55;
    endRangeText.alignment = ["left", "fill"];
    ...
    csvInputGroup.alignChildren = ["left", "fill"];
    csvInputGroup.spacing = 10;
    csvInputGroup.margins = 0;
    csvInputGroup.alignment = ["fill", "top"];

    // CSVINPUTGROUPTEXTGROUP
    // ======================
    var csvInputGroupTextGroup = csvInputGroup.add("group", undefined, { name: "csvInputGroupTextGroup" });
    csvInputGroupTextGroup.orientation = "column";
    csvInputGroupTextGroup.alignChildren = ["left", "center"];
    csvInputGroupTextGroup.spacing = 0;
    csvInputGroupTextGroup.margins = 0;

    var csvStaticText = csvInputGroupTextGroup.add("statictext", undefined, undefined, { name: "csvStaticText", truncate: "" });
    csvStaticText.helpTip = "Hit 'Edit' to select the CSV file.";
    csvStaticText.text = "CSV Input Path:";
    csvStaticText.preferredSize.width = 207;

    var csvInputPathText = csvInputGroupTextGroup.add("statictext", undefined, undefined, { name: "csvInputPathText", truncate: "middle" });
    csvInputPathText.helpTip = "Input path for the CSV file.";
    csvInputPathText.text = "No File Selected";
    csvInputPathText.preferredSize.width = 207;
    ...
    var selectCSVInputIconButton = csvInputGroup.add("iconbutton", undefined, File.decode(selectCSVInputIconButton_imgString), { name: "selectCSVInputIconButton", style: "button" });
    ...
    var unitNameText1 = unitNameGroup.add('edittext {properties: {name: "unitNameText1"}}' as 'edittext');
    unitNameText1.helpTip = "Always needed.";
    unitNameText1.text = "Unit Name Line 1";

    var unitNameText2 = unitNameGroup.add('edittext {properties: {name: "unitNameText2"}}' as 'edittext');
    unitNameText2.helpTip = "Can be left blank if not needed.";
    unitNameText2.text = "Unit Name Line 2";

    // CUSTOMINPUTPANEL
    // ================
    var endorserLineText = customInputPanel.add('edittext {properties: {name: "endorserLineText"}}' as 'edittext');
    ...
    outputGroup.alignChildren = ["left", "fill"];
    outputGroup.spacing = 10;
    outputGroup.margins = 0;
    outputGroup.alignment = ["left", "top"];

    // OUTPUTGROUPTEXTGROUP
    // ====================
    var outputGroupTextGroup = outputGroup.add("group", undefined, { name: "outputGroupTextGroup" });
    outputGroupTextGroup.orientation = "column";
    outputGroupTextGroup.alignChildren = ["left", "center"];
    outputGroupTextGroup.spacing = 0;
    outputGroupTextGroup.margins = 0;

    var outputStaticText = outputGroupTextGroup.add("statictext", undefined, undefined, { name: "outputStaticText", truncate: "" });
    outputStaticText.text = "Output Path:";
    outputStaticText.preferredSize.width = 249;

    var outputPathText = outputGroupTextGroup.add("statictext", undefined, undefined, { name: "outputPathText", truncate: "middle" });
    outputPathText.text = "No Folder Selected";
    outputPathText.preferredSize.width = 249;

    // OUTPUTGROUP
    // ===========
    var selectOututPathIconButton = outputGroup.add("iconbutton", undefined, File.decode(selectCSVInputIconButton_imgString), { name: "selectOututPathIconButton", style: "button" });
    ...
    progressPanel.alignChildren = ["left", "center"];
    progressPanel.spacing = 10;
    progressPanel.margins = 10;
    progressPanel.alignment = ["fill", "top"];

    var progressText = progressPanel.add('edittext {properties: {name: "progressText", readonly: true}}' as 'edittext');
    progressText.text = "0/0";
    progressText.alignment = ["left", "fill"];

    var progressbar = progressPanel.add("progressbar", undefined, undefined, 100, { name: "progressbar" });
    progressbar.maxvalue = 100;
    progressbar.value = 50;
    progressbar.preferredSize.width = 257;
    ...
    exportButton.onClick = function () {
        ...
        // 1. Check if the master document is open and retrieve it. Exit if not.
        ...
        // Set the colorspaces we need to use.
        let colorSpaceList: string[] = [];
        if (rgbCheckbox.value) {
            colorSpaceList.push('RGB');
        }
        if (cmykCheckbox.value) {
            colorSpaceList.push('CMYK');
        }

        if (colorSpaceList.length == 0) {
            alert('Either the RGB or CMYK (or both) checkboxes should be checked! Neither seem to be selected.')
            return;
        }

        for (let colorSpaceIndex = 0; colorSpaceIndex < colorSpaceList.length; colorSpaceIndex++) {

            const colorSpace = colorSpaceList[colorSpaceIndex];

            // Set the colorspace for the document and double check.
            // doc.documentColorSpace = DocumentColorSpace.RGB doesn't work, as this is a read-only property.
            if (colorSpace == 'RGB') {
                app.executeMenuCommand('doc-color-rgb');
                if (doc.documentColorSpace != DocumentColorSpace.RGB) {
                    alert("DocumentColorSpace is not as expected. Failed attempt to convert document to RGB colorspace. Please re-open the document, being careful not to save any changes.")
                    return;
                }
            } else if (colorSpace == 'CMYK') {
                app.executeMenuCommand('doc-color-cmyk');
                if (doc.documentColorSpace != DocumentColorSpace.CMYK) {
                    alert("DocumentColorSpace is not as expected. Failed attempt to convert document to CMYK colorspace. Please re-open the document, being careful not to save any changes.")
                    return;
                }
            } else {
                alert('Critical Error! Color space was somehow neither RGB nor CMYK. Please report to Oren.');
                return;
            }
            ...
            // Used for file folder exporting.
            options['colorSpace'] = colorSpace;

            // 4. Create the Color Schemes.
            const BerkeleyBlue = generateColors(colorSpace, 'blue')
            const CaliforniaGold = generateColors(colorSpace, 'gold')
            const White = generateColors(colorSpace, 'white')
